...
(async () => {
  ...
  const pubStatusLine = pubLines.find(line => line.includes('**Status Filter:**'));
  ...
  const formatTotalLine = formatLines.find(line => line.includes('**Total Publishers:**'));
  const formatStatusLine = formatLines.find(line => line.includes('**Status Filter:**'));
