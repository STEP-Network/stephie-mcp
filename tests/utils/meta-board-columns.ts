 ...
 */
export async function getMetaBoardConfig(boardKey: string): Promise<BoardColumnConfig | null> {
  ...
  try {
    ...
    const boardItem = items.find((item: any) => {
      ...
      const key = boardKey.toLowerCase().replace(/([A-Z])/g, ' $1').trim();
    ...
    columnValues.forEach((col: any) => {
      ...
      if (col.column.id === 'dropdown_mkvj2a8r' && col.value) {
        try {
          ...
          columnsConfig = col.text ? col.text.split(',').map((s: string) => s.trim()) : [];
        }
      }
    });
    
    console.log(`  Found board: ${boardItem.name} (ID: ${boardId})`);
    console.log(`  Configured columns: ${columnsConfig.length > 0 ? columnsConfig.join(', ') : 'None found'}`);
 ...
 */
export async function getBoardColumnDetails(boardId: string): Promise<Map<string, { title: string; type: string }>> {
  const query = `
    query {
      boards(ids: [${boardId}]) {
        columns {
  ...
  try {
    ...
    console.error(`Failed to fetch column details for board ${boardId}:`, error);
    return new Map();
  }
}
...
export async function getTestableColumns(
  ...
  if (!validation.valid) {
    if (validation.missing.length > 0) {
      warnings.push(`Tool uses columns not in meta config: ${validation.missing.join(', ')}`);
    }
    if (validation.unused.length > 0) {
      warnings.push(`Meta config has unused columns: ${validation.unused.join(', ')}`);
    }
  }
  
  // For testing, use the intersection of configured and used
  const validColumns = usedColumns.filter(col => 
    !metaConfig || metaConfig.columns.length === 0 || metaConfig.columns.includes(col)
  );
