...
export function validateFilteringEffect(
  ...
  if (baseline.totalItems === 0) {
    console.warn(`⚠️ WARNING: No items in baseline data for ${filterType} filter test. Board might be empty.`);
  ...
  if (filtered.itemIds.length > 0 && baseline.itemIds.length > 0) {
    ...
    const allFilteredInBaseline = filtered.itemIds.every(id => baselineSet.has(id));
...
export function validateSearchRelevance(
  ...
  if (baseline.totalItems === 0) {
    console.warn(`⚠️ WARNING: Cannot test search "${searchTerm}" - no items in baseline data.`);
...
export function validateStatusFilterResults(
  ...
  if (baseline.totalItems === 0) {
    console.warn(`⚠️ WARNING: Cannot test status filter - no items in baseline data.`);
    expect(itemCount).toBe(0);
    return;
  }
  
  // Check if this status exists in baseline
  if (baseline.statusDistribution && !Object.keys(baseline.statusDistribution).some(s => s.includes(statusLabel))) {
    if (itemCount === 0) {
      console.info(`ℹ️ INFO: Status "${statusLabel}" (${statusValue}) has no items - this appears correct based on baseline data.`);
    } else {
      console.warn(`⚠️ WARNING: Found ${itemCount} items for status "${statusLabel}" but this status wasn't in baseline data.`);
    }
  }
  ...
  if (itemCount > 0) {
    const statusRegex = new RegExp(`- \\*\\*Status:\\*\\*\\s+${statusLabel}`, 'gi');
...
export function validatePaginationWithWarnings(
  ...
  if (fullCount <= limit) {
    ...
    console.info(`ℹ️ INFO: Only ${fullCount} items available, limit of ${limit} cannot be properly tested.`);
    ...
    if (limitedCount > limit) {
      console.error(`❌ ERROR: Pagination broken - requested ${limit} items but got ${limitedCount}.`);
    }
  }
}
 ...
 */
export function performTestHealthCheck(baselineData: BaselineData, toolName: string): void {
...
export function validateRelationFilterEffect(
  ...
  if (baseline.totalItems === 0) {
    console.warn(`⚠️ WARNING: Cannot test ${relationName} filter - no items in baseline data.`);
    expect(itemCount).toBe(0);
    return;
  }
