...
import { validateToolOutput, extractItemCount } from './validators.js';
import { 
  DiscoveredTestData,
...
export function createDynamicTestSuite(config: DynamicTestConfig) {
  return describe(`${config.toolName} (Dynamic)`, () => {
    ...
    describe('Basic functionality', () => {
      ...
      it('should have sufficient test data', () => {
        ...
        if (discoveredData.totalItems === 0) {
          console.warn(`⚠️ ${config.toolName} has no test data - skipping dynamic tests`);
    ...
    if (discoveredData.hasData) {
        ...
        describe('Dynamic Search (with real content)', () => {
          for (const testCase of testCases.searchTests) {
            it(`should find results for "${testCase.term}"`, async () => {
              ...
              const containsTerm = lowerOutput.includes(testCase.term.toLowerCase());
              
              if (!containsTerm) {
                console.warn(`    ⚠️ Results don't visibly contain "${testCase.term}" - might be matching hidden fields`);
              } else {
                console.log(`    ✅ Found ${itemCount} items containing "${testCase.term}"`);
              }
            });
          }
        });
        ...
        describe('Dynamic Date Filters (with actual dates)', () => {
          for (const testCase of testCases.dateTests) {
            it(`should filter by ${testCase.field}="${testCase.date}"`, async () => {
              ...
              console.log(`    ✅ Found ${itemCount} items for ${testCase.field}="${testCase.date}"`);
            });
          }
        });
      }
            ...
            for (const limit of limits) {
              if (limit <= discoveredData.totalItems) {
                ...
                if (discoveredData.totalItems > limit) {
                  ...
                  console.log(`    ✅ Limit ${limit}: Got exactly ${limit} items`);
                } else {
                  expect(itemCount).toBe(discoveredData.totalItems);
                  console.log(`    ✅ Limit ${limit}: Got all ${discoveredData.totalItems} available items`);
                }
              }
            }
          });
      ...
      if (testCases.statusTests.length > 0 && testCases.searchTests.length > 0) {
        describe('Dynamic Combined Filters', () => {
          it('should handle multiple filters together', async () => {
            ...
            console.log(`    ✅ Combined ${statusTest.field}=${statusTest.value} + search="${searchTest.term}": ${itemCount} items`);
            
            // Even combined, we should get some results since both filters individually work
            if (itemCount === 0) {
              console.info(`    ℹ️ No overlap between status and search filters - this is valid`);
            }
          });
        });
      }
      ...
      describe('Performance', () => {
        it('should handle large result sets efficiently', async () => {
          ...
          console.log(`    ✅ Fetched ${limit} items in ${duration}ms (threshold: ${expectedMs}ms)`);
        });
      });
      ...
      it('should handle invalid filter values gracefully', async () => {
        ...
        for (const params of invalidTests) {
          try {
            ...
            console.log(`    ℹ️ Expected error for ${JSON.stringify(params)}: ${error.message}`);
          }
        }
      });
    });
