...
async function verifyMigration() {
  ...
  console.log(`  Found ${expectedColumns.size} boards with ${totalExpected} total column configurations\n`);
  ...
  console.log(`  Found ${columnItems.length} column items linked to ${createdColumns.size} boards\n`);
  ...
  for (const [boardItemId, expectedCols] of expectedColumns) {
    ...
    if (missing.length > 0) {
      ...
      console.log(`✅ ${boardName}: All ${expectedCols.size} columns created and linked`);
    }
  }
  
  // Step 4: Check for boards without any columns
  const boardsWithoutColumns: string[] = [];
  expectedColumns.forEach((cols, boardItemId) => {
    if (!createdColumns.has(boardItemId)) {
      boardsWithoutColumns.push(`${boardNames.get(boardItemId)} (${boardItemId})`);
    }
  });
  ...
  console.log(`  Success rate: ${((totalMatched / totalExpected) * 100).toFixed(1)}%`);
  
  if (allMatched) {
    console.log('\n✨ SUCCESS: All columns from dropdown_mkvj2a8r are created and linked!');
  } else {
    console.log('\n⚠️  Some columns are missing. This could be due to:');
    console.log('  1. Column names not matching exactly with actual board columns');
