...
async function runTests() {
  ...
  console.log(`Auth Token: ${TOKEN ? colors.green('Configured') : colors.red('Missing')}\n`);
  ...
  if (TOKEN) {
    await testEndpoint('Initialize (With Auth)', async () => {
      ...
      console.log(colors.gray(`  Server: ${data.result.serverInfo.name} v${data.result.serverInfo.version}`));
      if (data.result.user) {
    ...
    await testEndpoint('Call Tool (getAllPublishers)', async () => {
      ...
      console.log(colors.gray(`  Response: ${JSON.stringify(data.result).substring(0, 100)}...`));
    });
    ...
    console.log(colors.gray('  To run full tests, set TEST_AUTH_TOKEN in .env.local'));
    console.log(colors.gray('  Get a token from: https://ai.stepnetwork.dk/api-keys'));
  }
