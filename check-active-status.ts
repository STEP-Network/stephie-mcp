...
(async () => {
  ...
  for (const item of items) {
    ...
    const statusCol = item.column_values.find((col: any) => col.id === 'color_mkrqy0vi');
    const isActive = statusCol?.text === 'Aktiv' || statusCol?.text === 'Active';
  ...
  console.log('- Total with High-impact.js: ' + (highImpactActiveCount + highImpactInactiveCount));
})();