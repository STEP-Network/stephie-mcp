...
async function measurePerformance() {
  ...
  console.log(`   Overhead from dynamic columns: ~${colTime}ms (${Math.round(colTime/toolTime*100)}%)`);
  
  // Test 4: Cached performance (second call)
  console.log('\n4. CACHED PERFORMANCE:');
  const cachedStart = Date.now();
  const cachedColumns = await getDynamicColumns('1402911027');
  const cachedTime = Date.now() - cachedStart;
  console.log(`   Cached fetch time: ${cachedTime}ms`);
  console.log(`   Speed improvement: ${Math.round((1 - cachedTime/colTime) * 100)}%`);
