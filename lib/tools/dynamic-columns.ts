...
export async function getDynamicColumns(boardNameOrId: string): Promise<string[]> {
  ...
  try {
    ...
    if (/^\d+$/.test(boardNameOrId)) {
      ...
      const boardItem = metaItems.find((item: any) => {
      ...
      const boardItem = metaItems.find((item: any) => 
        item.name.toLowerCase() === boardNameOrId.toLowerCase() ||
        item.name.toLowerCase().includes(boardNameOrId.toLowerCase())
      );
    ...
    if (!boardItemId) {
      console.warn(`Board ${boardNameOrId} not found in meta board - using defaults`);
    ...
    const columnItems = columnsResponse.data?.boards?.[0]?.items_page?.items || [];
    ...
    if (columns.length === 0) {
      console.warn(`No columns found for board ${boardNameOrId} in Columns board - using defaults`);
      return getDefaultColumns(boardNameOrId);
    }
    ...
    console.error(`Failed to fetch dynamic columns for ${boardNameOrId}:`, error);
    return getDefaultColumns(boardNameOrId);
  }
}
...
function getDefaultColumns(boardName: string): string[] {
  const defaults: Record<string, string[]> = {
    'tasksMarketing': [
      "name", "person", "status_1__1", "color_mkpwc7hm", 
      "status_mkkw7ehb", "publish_date_mkn21n6b", "budget_mkn22001", 
      "title_mkn256dt", "link_to_teams_Mjj8UZOX", "link_to_teams_Mjj8FZuw", 
      "board_relation_mkpjg0ky", "budgets_mkn2xpkt"
    ],
    'accounts': [
      "text8", "status", "text", "phone", "people",
      "status5", "numbers", "status4", "color", "text5"
    ],
    // Add other boards...
  };
...
export function buildDynamicQuery(
  ...
  const queryParams = filters && filters.length > 0 
 ...
 */
export async function getTasksMarketingDynamic(params: any = {}) {
  ...
  // Build dynamic query
  const query = buildDynamicQuery('1693359113', columns, limit, filters);
...
function formatOutput(response: any): string {
  // Format logic here
