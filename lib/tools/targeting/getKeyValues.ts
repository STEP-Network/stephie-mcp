...
export async function getKeyValues(args: {
	...
	console.error(
		`[getKeyValues] Search: ${search}, Type: ${type}, Include values: ${includeValues}`,
	);
    ...
    `;
      ...
      `;
			...
			const valueItems =
				valueResponse.data?.boards?.[0]?.items_page?.items || [];
		...
		throw new Error(
			`Failed to fetch key values: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
}
