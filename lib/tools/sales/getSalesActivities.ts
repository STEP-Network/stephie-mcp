...
export async function getSalesActivities(params: {
  ...
  const { limit = 10, search, activity_owner, activity_status, activity_type, accountId, contactId, opportunityId } = params;
      ...
      boards(ids: [1402911042]) {
        ...
        items_page(limit: ${limit}${queryParams}) {
          items {
            ...
            created_at
            updated_at
            column_values(ids: ["name", "activity_owner", "activity_status", "activity_type", "connect_boards9__1", "link_to_contacts8__1", "link_to_opportunities__1"]) {
              id
              text
              value
              column {
  ...
  try {
    ...
    if (accountId) {
      items = items.filter((item: any) => {
        const relationCol = item.column_values.find((c: any) => c.id === 'connect_boards9__1');
    ...
    if (contactId) {
      items = items.filter((item: any) => {
        const relationCol = item.column_values.find((c: any) => c.id === 'link_to_contacts8__1');
        if (relationCol?.value) {
          try {
            const linked = JSON.parse(relationCol.value);
