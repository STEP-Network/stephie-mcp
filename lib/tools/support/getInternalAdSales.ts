...
export async function getInternalAdSales(
	params: {
		limit?: number;
		search?: string;
	...
	try {
		...
		return JSON.stringify(
			createListResponse(
				...
				metadata,
				{
					summary: `Found ${formattedItems.length} internal AdSales item${formattedItems.length !== 1 ? 's' : ''} (${dynamicColumns.length} dynamic columns)`
				}
			),
			null,
			2
		...
		console.error("Error fetching InternalAdSales items:", error);
		throw error;
	}
}
