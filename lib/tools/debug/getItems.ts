...
export async function getItems(args: {
	...
	try {
      ...
      `;
			...
			columns.forEach((col: Record<string, unknown>) => {
				columnTypes[col.id as string] = col.type as string;
			});
		}
		...
		if (itemIds && itemIds.length > 0) {
			...
			if (columnFilters) {
				columnFilters.forEach((filter) => {
					...
					if (
						(columnType === "status" || columnType === "dropdown") &&
						typeof filter.value === "string" &&
						Number.isNaN(Number(filter.value))
					) {
						throw new Error(
							`Column ${filter.columnId} is type '${columnType}' and requires a numeric index, not text value "${filter.value}". Use getBoardColumns first to see the index mapping.`,
						);
					}
					...
					if (operator && typeof operator === "string") {
						...
						if (typeOperators?.[operator]) {
							operator = typeOperators[operator];
						} else {
							console.error(
								`[getItems] Warning: Operator "${operator}" not found for column type "${columnType}", using default`,
							);
						...
						if (typeOperators) {
							...
							if (
								...
								if (
									...
									filter.value.every((v) => !Number.isNaN(Number(v)))
								) {
								...
								operator = filter.value
									? typeOperators.checked
									: typeOperators.unchecked;
					...
					let compareValue: string | number | boolean | (string | number | boolean)[] = filter.value;
					...
					if (
						(operator === "any_of" || operator === "not_any_of") &&
				...
				const rulesString = rules
					.map((rule) => {
						...
						if (Array.isArray(rule.compare_value)) {
							...
							compareValue = rule.compare_value as string;
						}
    ...
    `;
		...
		if (itemIds)
			console.error(`[getItems] Specific item IDs: ${itemIds.join(", ")}`);
		if (columnIds)
			console.error(`[getItems] Specific column IDs: ${columnIds.join(", ")}`);
		if (columnFilters) {
			console.error(
				`[getItems] Column filters:`,
				columnFilters.map((f) => ({
					column: f.columnId,
		...
		const processedItems = (items as MondayItemResponse[]).map(
			(item: MondayItemResponse) => {
				...
				item.column_values?.forEach((col: MondayColumnValueResponse) => {
					...
					let value: string | number =
						col.text || col.value || col.display_value || "";
		...
		if (includeColumnMetadata && board.columns) {
			result.boardColumns = board.columns.map(
				(col: Record<string, unknown>) => ({
					id: col.id,
					title: col.title,
					type: col.type,
					settings: col.settings_str
						? JSON.parse(col.settings_str as string)
						: null,
				}),
			);
		}
		...
		throw new Error(
			`Failed to fetch items: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
}
