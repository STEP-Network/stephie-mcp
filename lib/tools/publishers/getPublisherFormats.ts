...
function parseDevices(value: string | undefined): string[] | null {
  ...
  parts.forEach(part => {
    ...
    } else if (part.toLowerCase().includes('mobil') || part.toLowerCase().includes('mobile')) {
      devices.push('mobil');
  ...
  try {
    ...
    const publishers = items.map((item: any) => {
      ...
      const formats = {
        ...
        videoFunction: columnValues.color_mkr4s6rs?.value === 'Live',
        ott: columnValues.color_mkr4rzd3?.value === 'Live',
        readStatus: columnValues.color_mksdc9rp?.value === 'Aktiv',

        // Device-based formats with their available devices
        topscrollAdnami: parseDevices(columnValues.dropdown_mksd7frz?.value),
        topscrollExpandAdnami: parseDevices(columnValues.dropdown_mksdjeft?.value),
        doubleMidscrollAdnami: parseDevices(columnValues.dropdown_mksdbwbf?.value),
        midscrollAdnami: parseDevices(columnValues.dropdown_mksd17vw?.value),
        adnamiNative: parseDevices(columnValues.dropdown_mksdb150?.value),
        
        topscrollHighImpact: parseDevices(columnValues.dropdown_mksdcgvj?.value),
        midscrollHighImpact: parseDevices(columnValues.dropdown_mksdjpqx?.value),
        
        wallpaper: parseDevices(columnValues.dropdown_mksdytf0?.value),
        anchor: parseDevices(columnValues.dropdown_mksdr0q2?.value),
        trueNative: parseDevices(columnValues.dropdown_mksdh745?.value),
        googleInterstitial: parseDevices(columnValues.dropdown_mksdfx54?.value),
        outstream: parseDevices(columnValues.dropdown_mksd6yy?.value),
        video: parseDevices(columnValues.dropdown_mksddmgt?.value),
        vertikalVideo: parseDevices(columnValues.dropdown_mksdw0qh?.value),
      };

      return {
        id: item.id,
        name: item.name,
        publisherGroup: columnValues.board_relation__1?.value || '',
        formats: formats,
    ...
    textLines.push('');
    textLines.push('> **Note:** Only ACTIVE formats shown. If a format is not listed = NOT available.');
    textLines.push('> **Devices:** M=Mobile, D=Desktop, A=App, D-spec=Desktop (non-programmatic), M-spec=Mobile (non-programmatic)');
    textLines.push('');
    ...
    sortedGroups.forEach((groupName) => {
      ...
      groupPublishers.forEach((publisher: any) => {
        ...
        publisher.availableFormats.deviceFormats.forEach((item: any) => {
          const devices = item.devices
            .map((d: string) => deviceNames[d] || d)
            .join('/');
          formats.push(`${formatNames[item.format] || item.format}[${devices}]`);
        });
    ...
    throw new Error(`Failed to fetch publisher formats: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}