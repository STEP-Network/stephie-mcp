...
export async function updateTaskAdTech(params: {
  itemId: string;
  name?: string;
  ...
  const mutation = `
    mutation UpdateTaskAdTech($columnValues: JSON!) {
      ${mutations.join('\n')}
    }
  `;
  ...
  try {
    ...
    console.error('Error updating AdTech task:', error);
    return `# Error\n\nFailed to update AdTech task: ${error}`;
  }
}