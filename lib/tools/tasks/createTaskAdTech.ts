...
export async function createTaskAdTech(params: {
	name: string;
	status?: number; // Status: 0=Done, 2=In Progress, 3=New, 5=On Hold, 6=Waiting, 7=Blocked
	...
	const mutation = `
    mutation CreateTaskAdTech($boardId: ID!, $name: String!, $columnValues: JSON!, $groupId: String) {
      create_item(
        board_id: $boardId,
        item_name: $name,
  ...
  `;
	...
	const variables = {
		boardId: TOOL_BOARD_IDS.TASKS_AD_TECH,
		name,
	...
	try {
		...
		if (response.errors) {
			...
			return `# Error Creating AdTech Task\n\n${response.errors.map((e: GraphQLError) => e.message).join("\n")}`;
		}

		const item = response.data?.create_item;
		if (!item) {
			return "# Error\n\nFailed to create AdTech task - no item returned";
		}

		// Format response
		let result = `# AdTech Task Created Successfully\n\n`;
		...
		console.error("Error creating AdTech task:", error);
		return `# Error\n\nFailed to create AdTech task: ${error}`;
	}
}
