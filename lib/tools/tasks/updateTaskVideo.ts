...
export async function updateTaskVideo(params: {
	itemId: string;
	name?: string;
	...
	const mutation = `
    mutation UpdateTaskVideo($columnValues: JSON!) {
      ${mutations.join("\n")}
    }
  `;
	...
	try {
		...
		return `# Error\n\nFailed to update Video task: ${error}`;
	}
}
