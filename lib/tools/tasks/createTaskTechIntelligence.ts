...
export async function createTaskTechIntelligence(params: CreateParams) {
	...
	if (fields.person !== undefined) {
		columnValues.person = {
			personsAndTeams: [{ id: parseInt(fields.person, 10), kind: "person" }],
		};
	}
  ...
  `;
	...
	try {
		...
		lines.push(`**Item ID:** ${(createdItem as any).id}`);
		lines.push(`**Name:** ${(createdItem as any).name}`);
		lines.push(`**Created At:** ${(createdItem as any).created_at}`);
		lines.push("");
		lines.push("## Fields");

		(createdItem as any).column_values.forEach(
