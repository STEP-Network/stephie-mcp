...
export async function getProcessesItems(params: {
  limit?: number;
  search?: string;
      ...
      boards(ids: [1611396339]) {
        ...
        items_page(limit: ${limit}${queryParams}) {
          items {
            ...
            created_at
            updated_at
            column_values(ids: ["name", "people__1", "connect_boards__1", "status"]) {
              id
              text
              value
              column {
  ...
  try {
    ...
    console.error('Error fetching Processes items:', error);
    throw error;
  }
}
