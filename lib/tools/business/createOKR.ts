...
export async function createOKR(params: {
	...
	const { name, status, people, numbers, date, long_text, teamId, groupId } =
		params;
  ...
  `;
	...
	try {
		...
		const item = response.data?.create_item as MondayItemResponse | undefined;
		if (!item) {
