...
export async function getPlatformsItems(params: {
  limit?: number;
  search?: string;
  people__1?: string; // Owner
  status?: number; // Status (numeric index)
} = {}) {
  const { limit = 10, search, people__1, status } = params;
      ...
      boards(ids: [1611329866]) {
        ...
        items_page(limit: ${limit}${queryParams}) {
          items {
            ...
            created_at
            updated_at
            column_values(ids: ["name", "people__1", "status", "connect_boards__1", "link_to_tasks___video__1"]) {
              id
              text
              value
              column {
  ...
  try {
    ...
    console.error('Error fetching Platforms items:', error);
    throw error;
  }
}
