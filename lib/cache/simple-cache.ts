...
class SimpleCache {
  ...
  private processResponse(response: any): CacheMetadata {
    ...
    columnsBoard.items_page?.items?.forEach((colItem: any) => {
      const columnId = colItem.column_values.find((c: any) => c.id === 'text_mkvjc46e')?.text;
      const boardRelation = colItem.column_values.find((c: any) => c.id === 'board_relation_mkvjb1w9');
