...
async function testCachePerformance() {
  ...
  console.log(`   Cache read: ${cacheTime}ms (${columnsFromCache.length} columns)`);
  ...
  console.log(`   Dynamic columns: ${dynamicTime}ms (${dynamicColumns.length} columns)`);
  ...
  const cachedColumns = await getDynamicColumns(boardId);
  ...
  console.log(`   Performance improvement: ${Math.round((1 - dynamicTime/3900) * 100)}%`);
  
  // Check cache metadata
  const metadata = await cache.getMetadata();
  console.log('\n📦 CACHE STATS:');
  console.log(`   Boards cached: ${Object.keys(metadata?.columns || {}).length}`);
  console.log(`   Total columns: ${Object.values(metadata?.columns || {}).reduce((sum, cols) => sum + cols.length, 0)}`);
  console.log(`   Last sync: ${metadata?.lastSync}`);
  console.log(`   Cache location: ${process.env.VERCEL ? '/tmp/cache' : './cache'}`);
}
