...
(async () => {
  ...
  const response = await mondayApi(query, { boardId: '1222800432', limit: 500 });
  ...
  for (const item of items) {
    const topscrollCol = item.column_values.find((col: any) => col.id === 'dropdown_mksdcgvj');
    const midscrollCol = item.column_values.find((col: any) => col.id === 'dropdown_mksdjpqx');
    ...
    if (topscrollCol?.value) {
      try {
        ...
        if (parsed.ids && (parsed.ids.includes(108) || parsed.ids.includes(20))) {
          hasHighImpact = true;
        }
      } catch (e) {}
    }
    ...
    if (midscrollCol?.value) {
      try {
        ...
        if (parsed.ids && (parsed.ids.includes(108) || parsed.ids.includes(4))) {
          hasHighImpact = true;
        }
      } catch (e) {}
    }
  ...
  for (const pubName of checkPublishers) {
    ...
    if (pub) {
      console.log('\n' + pubName + ':');
      const topscrollCol = pub.column_values.find((col: any) => col.id === 'dropdown_mksdcgvj');
      const midscrollCol = pub.column_values.find((col: any) => col.id === 'dropdown_mksdjpqx');
      
      console.log('  Topscroll High-impact: ' + topscrollCol?.text + ' (value: ' + topscrollCol?.value + ')');
      console.log('  Midscroll High-impact: ' + midscrollCol?.text + ' (value: ' + midscrollCol?.value + ')');
    }
  }
})();