...
const BOARD_ID_MAP: Record<string, string> = {
  ...
  'getPlatforms': '1682223428',
  'getProcesses': '1682223451',
...
async function migrateFile(filePath: string): Promise<boolean> {
  ...
  if (!content.includes("import { getDynamicColumns }")) {
    ...
    if (imports) {
      ...
      const depth = filePath.split('/').filter(p => p === 'tools').length + 
                     filePath.split('/').filter(p => p !== 'tools' && p !== 'lib' && p.includes('/')).length;
  ...
  if (functionMatch) {
    ...
    const boardIdInFile = content.match(/boards\(ids:\s*\[(\d+)\]/)?.[1] || boardId;
  ...
  `;
      ...
      content = content.slice(0, paramsEndIndex) + dynamicColumnsCode + content.slice(paramsEndIndex);
