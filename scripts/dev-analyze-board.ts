      ...
      limit: { type: 'number', default: 10 },`);
		...
		filterableColumns.slice(0, 5).forEach((col) => {
			if (
				...
				console.log(
					`      ${col.id.replace(/-/g, "_")}: { type: 'number', description: '${col.title} (index)' },`,
				);
			} else if (col.type === "checkbox") {
				console.log(
					`      ${col.id.replace(/-/g, "_")}: { type: 'boolean', description: '${col.title}' },`,
				);
			} else if (col.type === "date") {
				console.log(
					`      ${col.id.replace(/-/g, "_")}: { type: 'string', description: '${col.title} (YYYY-MM-DD)' },`,
				);
			} else {
				console.log(
					`      ${col.id.replace(/-/g, "_")}: { type: 'string', description: '${col.title}' },`,
				);
			}
		});
		...
		allColumns.forEach((col) => {
			console.log(
				`- ${col.title} (${col.id}) - Type: ${col.type}${col.hasOptions ? ` [${col.optionCount} options]` : ""}`,
			);
		});
	} catch (error) {
