...
async function updateBoardItems(allColumnNames: string[]) {
	...
	for (const info of boardColumnInfo) {
		...
		if (!itemId) {
			console.log(
				`⚠️  No item found for board ${info.boardName} (${info.boardId})`,
			);
			continue;
		}

		// Get indices for each column name
		const indices = Array.from(info.columnNames)
			.map((name) => allColumnNames.indexOf(name))
			.filter((idx) => idx >= 0);
    ...
    `;
		...
		try {
			...
			console.log(
				`✅ Updated ${info.boardName}: ${Array.from(info.columnNames).slice(0, 5).join(", ")}${info.columnNames.size > 5 ? "..." : ""}`,
			);
		} catch (error) {
