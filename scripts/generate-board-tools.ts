...
async function generateBoardTools() {
	...
	for (const board of BOARDS_TO_GENERATE) {
		...
		try {
			...
			const { essential, filterable } = selectEssentialColumns(
				boardData.columns,
			);
...
`;
			...
			toolFunctions.push(
				`export { ${functionName} } from './tools/${board.category}/${functionName}.js';`,
			);
							...
							filterable.map((f) => {
								...
								if (
									...
									return [
										...
										{
											type: "number",
											description: `${f.title} (numeric index)`,
										},
									];
								} else if (f.type === "checkbox") {
									...
									return [
										propName,
										{ type: "string", description: `${f.title} (YYYY-MM-DD)` },
									];
								} else {
