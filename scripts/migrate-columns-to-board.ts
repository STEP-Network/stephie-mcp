...
async function fetchMetaBoardConfigs(): Promise<BoardConfig[]> {
  ...
  for (const item of items) {
    const dropdownCol = item.column_values.find((c: any) => c.id === 'dropdown_mkvj2a8r');
...
async function fetchBoardColumns(boardId: string): Promise<ColumnDetail[]> {
  ...
  try {
    ...
    console.error(`    ❌ Failed to fetch columns for board ${boardId}:`, error.message);
    return [];
  }
}

async function checkColumnExists(columnId: string): Promise<boolean> {
  const query = `
    query {
...
async function loadExistingColumns(): Promise<Set<string>> {
  ...
  try {
    ...
    items.forEach((item: any) => {
      ...
      const colIdField = item.column_values.find((c: any) => c.id === 'text_mkvjc46e');
...
async function createColumnItem(
  ...
  try {
    ...
    if (createdItem) {
      console.log(`    ✅ Created: "${columnName}" (${columnId}) linked to ${boardName}`);
      return true;
    }
    return false;
  } catch (error) {
...
async function migrateColumns() {
  ...
  for (const config of boardConfigs) {
    ...
    for (const dropdownName of config.dropdownColumns) {
      ...
      let column = boardColumns.find(col => 
        col.title.toLowerCase() === searchName.toLowerCase()
      );
      
      // Try partial match if exact match fails
      if (!column) {
        column = boardColumns.find(col => 
      ...
      if (column) {
        ...
        const similar = boardColumns.filter(col => 
          col.title.toLowerCase().includes(searchName.substring(0, 3).toLowerCase())
        );
        if (similar.length > 0) {
          console.log(`     Possible matches: ${similar.map(c => `"${c.title}" (${c.id})`).join(', ')}`);
        }
      }
    }
    ...
    if (matches.length > 0) {
      ...
      for (const match of matches) {
        ...
        if (alreadyExists) {
          console.log(`    ⏭️  Skipped: "${match.name}" (${match.id}) - already exists for this board`);
          totalSkipped++;
        } else {
  ...
  if (totalCreated > 0) {
    ...
    console.log('   Each item is linked to its parent board and contains the column ID.');
  }
}
