...
async function listAllBoards() {
  ...
  try {
    ...
    const boardData = boards.map((item: any) => {
    ...
    boardData.forEach((board: any) => {
      console.log(`| ${board.name} | ${board.id} | ${board.type} | ${board.status} | ${board.priority} |`);
    });
    ...
    console.log(JSON.stringify(boardData.filter((b: any) => b.id), null, 2));
