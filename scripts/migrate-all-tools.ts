...
function migrateToolFile(filePath: string): boolean {
  ...
  const dirDepth = filePath.split('/').length - filePath.split('/').indexOf('tools') - 1;
  ...
  if (functionMatch) {
    ...
    const funcStart = content.indexOf(functionMatch[0]) + functionMatch[0].length;
    const nextLine = content.indexOf('\n', funcStart);
    
    // Look for the const destructuring line
    const destructuringMatch = content.slice(funcStart).match(/^\s*const\s*{[^}]+}\s*=\s*params;?\n/m);
    let insertPos;
    
    if (destructuringMatch) {
      insertPos = funcStart + destructuringMatch.index! + destructuringMatch[0].length;
    } else {
...
`;
    ...
    content = content.slice(0, insertPos) + dynamicCode + content.slice(insertPos);
    ...
    if (columnMatch) {
      ...
      const dynamicReplacement = 'column_values(ids: [${dynamicColumns.map(id => `"${id}"`).join(", ")}])';
      ...
      if (queryStart !== -1 && queryEnd !== -1) {
        ...
        content = content.slice(0, queryStart) + queryContent + content.slice(queryEnd + 2);
      }
    }
  }
