...
async function getExistingColumns(): Promise<ColumnInfo[]> {
  ...
  `;
	...
	for (const item of items) {
		const columnId = item.column_values.find(
		...
		if (columnId && boardItemId) {
			...
			for (const [_key, config] of boardMappings) {
...
async function removeOrphanedColumns() {
	...
	for (const column of orphaned) {
		console.log(
			`    - ${column.title} (${column.id}) from ${column.boardName}`,
		);
	}
...
async function main() {
	...
	switch (command) {
		...
		default:
			...
			console.log(
				"  pnpm columns:report         - Generate configuration report",
			);
			console.log(
				"  pnpm columns:sync <board>   - Add missing columns for a board",
			);
