...
const PRIORITY_RELATIONS = {
  // CRM Core Relations
  getAccounts: [
    { param: 'contactsId', column: 'account_contact', title: 'Contacts', description: 'Filter by linked contacts (use getContacts to find IDs)' },
    { param: 'opportunitiesId', column: 'account_deal', title: 'Opportunities', description: 'Filter by linked opportunities (use getOpportunities to find IDs)' },
    { param: 'leadsId', column: 'connect_boards9', title: 'Leads', description: 'Filter by linked leads (use getLeads to find IDs)' }
  ],
  getContacts: [
    { param: 'accountId', column: 'contact_account', title: 'Account', description: 'Filter by linked account (use getAccounts to find IDs)' },
    { param: 'opportunitiesId', column: 'contact_deal', title: 'Opportunities', description: 'Filter by linked opportunities (use getOpportunities to find IDs)' }
  ],
  getOpportunities: [
    { param: 'accountId', column: 'connect_boards31', title: 'Advertiser/Publisher', description: 'Filter by linked account (use getAccounts to find IDs)' },
    { param: 'contactId', column: 'deal_contact', title: 'Agency Contact', description: 'Filter by linked contact (use getContacts to find IDs)' },
    { param: 'bookingId', column: 'connect_boards8__1', title: 'Booking', description: 'Filter by linked booking (use getBookings to find IDs)' }
  ],
  getDeals: [
    { param: 'agencyId', column: 'connect_boards_mkmjpjjc', title: 'Agency', description: 'Filter by agency account (use getAccounts to find IDs)' },
    { param: 'advertiserId', column: 'connect_boards_mkmjr3e3', title: 'Advertiser', description: 'Filter by advertiser account (use getAccounts to find IDs)' },
    { param: 'contactsId', column: 'connect_boards3__1', title: 'Contacts', description: 'Filter by linked contacts (use getContacts to find IDs)' }
  ],
  
  // Task Relations (Key Results are critical)
  getTasksTechIntelligence: [
    { param: 'keyResultId', column: 'board_relation_mkpjqgpv', title: 'Key Result', description: 'Filter by linked key result (use OKR subitems to find IDs)' },
    { param: 'teamTaskId', column: 'connect_boards_Mjj8XLFi', title: 'Team Task', description: 'Filter by linked team tasks' }
  ],
  getTasksAdOps: [
    { param: 'keyResultId', column: 'board_relation_mkpjy03a', title: 'Key Result', description: 'Filter by linked key result (use OKR subitems to find IDs)' },
    { param: 'publisherId', column: 'connect_boards_mkkxdfax', title: 'Publisher', description: 'Filter by publisher (use getAllPublishers to find IDs)' }
  ],
  getTasksMarketing: [
    { param: 'keyResultId', column: 'board_relation_mkpjg0ky', title: 'Key Result', description: 'Filter by linked key result (use OKR subitems to find IDs)' },
    { param: 'budgetId', column: 'budgets_mkn2xpkt', title: 'Budget', description: 'Filter by linked budget (use getMarketingBudgets to find IDs)' }
  ],
  
  // OKR Relations (beyond team)
  getOKR: [
    { param: 'strategiesId', column: 'link_to_strategies__1', title: 'Strategies', description: 'Filter by linked strategies (use getStrategies to find IDs)' },
    { param: 'peopleId', column: 'connect_boards35__1', title: 'People', description: 'Filter by linked people (use getPeople to find IDs)' }
  ],
  
  // Other important relations
  getTickets: [
    { param: 'contactId', column: 'connect_boards8__1', title: 'Contacts', description: 'Filter by linked contact (use getContacts to find IDs)' },
    { param: 'assignedId', column: 'connect_boards__1', title: 'Assigned', description: 'Filter by assigned person (use getPeople to find IDs)' },
    { param: 'publisherId', column: 'connect_boards08__1', title: 'Publisher', description: 'Filter by publisher (use getAllPublishers to find IDs)' }
  ]
};
...
async function updateToolDefinitions() {
  ...
  const boardDefPath = path.join(__dirname, '../lib/mcp/boardToolDefinitions.json');
  ...
  for (const [toolName, relations] of Object.entries(PRIORITY_RELATIONS)) {
    ...
    const toolDefRegex = new RegExp(`name: '${toolName}'[^}]+inputSchema: {[^}]+properties: {([^}]+)}`, 's');
